...
CORE_POWER_W = 20e6  # Assuming a 3200 MWt reactor for a PWR example
...
def process_spectrum(sp_file: str, name: str):
    ...
    try:
        with openmc.StatePoint(sp_file) as sp:
            ...
            t_fiss = sp.get_tally(name='fiss_prod') # Get the fission production tally
            
            # Get the total simulated neutron production rate (P_sim)
            # Tally score is P_sim = <nu-fission * Phi> (neutrons/source particle)
            # This is the integrated score over the core volume.
            P_sim = t_fiss.mean[0]
            
            # The required actual production rate in the core (P_actual)
            # P_actual = Power / (Energy per neutron production)
            # Since the score is nu-fission, this factor is:
            P_actual = CORE_POWER_W / E_FISS_J # [W / (J/fission)] = [fissions/s]
            
            # Normalization Factor N: (Actual total fissions/s) / (Simulated total nu-fission score)
            N = P_actual / P_sim
            ...
            # Apply Normalization N to get units of [neutrons/cm^2/s]
            ...
            print(f"Total Fission Production Score (nu-fission): {P_sim:.4e} $\pm$ {t_fiss.std_dev[0]:.2e}")
            print(f"Normalization Factor N (applied to flux): {N:.2e}")
...
######
#Flux unit making normal not bullshit
...
if not df_aci.empty and not df_aco.empty:
    ...
    plt.ylabel('E $\Phi$(u) (Flux per Unit Lethargy) [neutrons/(cm^2*s)]')
