...
# ----------------------------------------------------------------------
#can change the temp here to change from czp or hfp
T_ref = 600 # K
...
def process_spectrum(sp_file: str, name: str):
    ...
    try:
        with openmc.StatePoint(sp_file) as sp:
            ...
            #Compute simulated fission production and the uncert.
            # t_fiss.mean and t_fiss.std_dev are arrays
            ...
            area = np.sum(flux_per_u_norm * delta_u)
            flux_per_u_norm = flux_per_u_norm / area
...
#the above sp_ac... files had a lot of issues with language i think, 


#Post-Process Results (Applying the string/path fix)
...
#Plot Flux Spectrum Per Unit Lethargy

if not df_aci.empty and not df_aco.empty:
    plt.figure(figsize=(10, 6))
    
    #Fixed the issue of the axis not being norm'd to one
    plt.loglog(df_aci['E_mid [eV]'], df_aci['flux_per_u_norm'], 
               label=f'ACI (Controls In, $k_{{eff}}$={df_aci["k_eff"].iloc[0]:.4f})', linewidth=2)
    plt.loglog(df_aco['E_mid [eV]'], df_aco['flux_per_u_norm'], 
               label=f'ACO (Controls Out, $k_{{eff}}$={df_aco["k_eff"].iloc[0]:.4f})', linewidth=2)

    plt.xlabel('Energy E [eV]')
    plt.ylabel(r'$\Phi(u)$ with $\int \Phi(u)\,du = 1$')

    #plt.ylabel(r'$E \, \Phi(u)$  (Flux per Unit Lethargy)   [n/cm$^{2}$\ s]')
    plt.title('Core-Average Normalized Neutron Flux Spectrum (ACI vs ACO) at 600 K') #remeber to change the temperature here and the name o fthe figure in line 350 when trying to plot the thing
    plt.grid(True, which='both', ls='--')
    plt.legend()
    plt.tight_layout()
    plt.show()
    plt.savefig('HFPphi.png')
